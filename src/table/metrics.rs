...
//! - `nanostore.table.<engine>.get_duration` - Get latency histogram
//! - `nanostore.table.<engine>.put_duration` - Put latency histogram
//! - `nanostore.table.<engine>.delete_duration` - Delete latency histogram
//! - `nanostore.table.<engine>.scan_duration` - Scan latency histogram
//!
...
///
/// Usage: `record_latency!("nanostore.table.btree.get_duration", start_time);`
#[macro_export]
...
pub fn record_put_duration(engine: &str, start: Instant) {
    histogram!(format!("nanostore.table.{}.put_duration", engine))
        .record(start.elapsed().as_secs_f64());
}
...
pub fn record_delete_duration(engine: &str, start: Instant) {
    histogram!(format!("nanostore.table.{}.delete_duration", engine))
        .record(start.elapsed().as_secs_f64());
}
...
pub fn record_scan_duration(engine: &str, start: Instant) {
    histogram!(format!("nanostore.table.{}.scan_duration", engine))
        .record(start.elapsed().as_secs_f64());
}
...
pub mod btree {
    ...
    #[inline]
    pub fn record_split() {
        counter!("nanostore.table.btree.split").increment(1);
    }

    /// Record a node merge operation.
    #[inline]
    pub fn record_merge() {
        counter!("nanostore.table.btree.merge").increment(1);
    }

    /// Record a node read operation.
    #[inline]
    pub fn record_node_read() {
        counter!("nanostore.table.btree.node_read").increment(1);
    }

    /// Record a node write operation.
    #[inline]
    pub fn record_node_write() {
        counter!("nanostore.table.btree.node_write").increment(1);
    }

    /// Update the current tree height.
    #[inline]
    pub fn set_tree_height(height: usize) {
        gauge!("nanostore.table.btree.tree_height").set(height as f64);
    }

    /// Update the number of internal nodes.
    #[inline]
    pub fn set_internal_nodes(count: u64) {
        gauge!("nanostore.table.btree.internal_nodes").set(count as f64);
    }

    /// Update the number of leaf nodes.
    #[inline]
    pub fn set_leaf_nodes(count: u64) {
        gauge!("nanostore.table.btree.leaf_nodes").set(count as f64);
    }
...
pub mod rtree {
    ...
    #[inline]
    pub fn record_split() {
        counter!("nanostore.table.rtree.split").increment(1);
    }

    /// Record a spatial query operation.
    #[inline]
    pub fn record_query() {
        counter!("nanostore.table.rtree.query").increment(1);
    }

    /// Record the number of candidate nodes examined during a query.
    #[inline]
    pub fn record_query_candidates(count: usize) {
        histogram!("nanostore.table.rtree.query_candidates").record(count as f64);
    }

    /// Record the number of results returned from a query.
    #[inline]
    pub fn record_query_results(count: usize) {
        histogram!("nanostore.table.rtree.query_results").record(count as f64);
    }

    /// Update the current tree height.
    #[inline]
    pub fn set_tree_height(height: u32) {
        gauge!("nanostore.table.rtree.tree_height").set(height as f64);
    }

    /// Update the number of objects indexed.
    #[inline]
    pub fn set_object_count(count: usize) {
        gauge!("nanostore.table.rtree.object_count").set(count as f64);
    }

    /// Record spatial query latency.
    #[inline]
    pub fn record_query_duration(start: Instant) {
        histogram!("nanostore.table.rtree.query_duration").record(start.elapsed().as_secs_f64());
    }
}
...
pub mod timeseries {
    ...
    pub fn record_aggregation_duration(start: Instant) {
        histogram!("nanostore.table.timeseries.aggregation_duration")
            .record(start.elapsed().as_secs_f64());
    }
}
...
pub mod hnsw {
    ...
    #[inline]
    pub fn record_insert() {
        counter!("nanostore.table.hnsw.insert").increment(1);
    }

    /// Record a vector search operation.
    #[inline]
    pub fn record_search() {
        counter!("nanostore.table.hnsw.search").increment(1);
    }

    /// Record the number of distance calculations during search.
    #[inline]
    pub fn record_distance_calculations(count: usize) {
        histogram!("nanostore.table.hnsw.distance_calculations").record(count as f64);
    }

    /// Record the number of hops during search.
    #[inline]
    pub fn record_search_hops(count: usize) {
        histogram!("nanostore.table.hnsw.search_hops").record(count as f64);
    }

    /// Update the number of vectors indexed.
    #[inline]
    pub fn set_vector_count(count: usize) {
        gauge!("nanostore.table.hnsw.vector_count").set(count as f64);
    }

    /// Update the maximum layer in the graph.
    #[inline]
    pub fn set_max_layer(layer: usize) {
        gauge!("nanostore.table.hnsw.max_layer").set(layer as f64);
    }

    /// Record vector search latency.
    #[inline]
    pub fn record_search_duration(start: Instant) {
        histogram!("nanostore.table.hnsw.search_duration").record(start.elapsed().as_secs_f64());
    }
}
