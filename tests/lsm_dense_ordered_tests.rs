...
fn test_lsm_dense_ordered_prefix_scan() {
    ...
    lsm.insert_entry(b"age:25", b"user:1", tx_id, lsn).unwrap();
    lsm.insert_entry(b"age:30", b"user:2", tx_id, lsn).unwrap();
    lsm.insert_entry(b"name:alice", b"user:3", tx_id, lsn).unwrap();
    lsm.insert_entry(b"name:bob", b"user:4", tx_id, lsn).unwrap();
...
fn test_lsm_dense_ordered_delete() {
    ...
    lsm.delete_entry(b"age:30", b"user:2", tx_id2, lsn2).unwrap();
...
fn test_lsm_dense_ordered_mvcc() {
    ...
    lsm.insert_entry(b"age:25", b"user:1", tx_id1, lsn1).unwrap();
    
    // Update entry at LSN 2
    let tx_id2 = TransactionId::from(2);
    let lsn2 = LogSequenceNumber::from(2);
    lsm.insert_entry(b"age:25", b"user:2", tx_id2, lsn2).unwrap();
...
fn test_lsm_dense_ordered_stats() {
    ...
    for i in 0..10 {
        ...
        lsm.insert_entry(key.as_bytes(), val.as_bytes(), tx_id, lsn).unwrap();
    }
