...
fn test_bloom_filter_rollback_visibility() {
    ...
    bloom
        .add_tombstone(b"key1".to_vec(), TransactionId::from(1))
        .unwrap();
...
fn test_bloom_filter_tombstone_vacuum() {
    ...
    bloom
        .add_tombstone(b"key2".to_vec(), TransactionId::from(2))
        .unwrap();
    bloom.commit_tombstones(TransactionId::from(2), LogSequenceNumber::from(200));

    bloom
        .add_tombstone(b"key3".to_vec(), TransactionId::from(3))
        .unwrap();
...
fn test_bloom_filter_multiple_tombstones() {
    ...
    for i in 0..10 {
        ...
        bloom
            .insert(
                ...
                LogSequenceNumber::from(100 + i as u64),
            )
            .unwrap();
    }
    ...
    for i in 0..5 {
        let key = format!("key{}", i);
        bloom
            .add_tombstone(key.as_bytes().to_vec(), TransactionId::from(i))
            .unwrap();
        bloom.commit_tombstones(
            TransactionId::from(i),
            LogSequenceNumber::from(200 + i as u64),
        );
    }
